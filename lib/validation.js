...
export const businessPlanSchema = z.object({
  ...
  values: z.string().max(500).optional().transform(val => val ?? ''),
  ...
  marketSize: z.string().max(200).optional().transform(val => val ?? ''),
  ...
  indirectCompetitors: z.string().max(500).optional().transform(val => val ?? ''),
  competitiveAdvantage: z.string().min(10, 'Vantaggio competitivo richiesto').max(1000),
  barriers: z.string().max(500).optional().transform(val => val ?? ''),
  ...
  roadmap: z.string().max(1000).optional().transform(val => val ?? ''),
  ...
  cac: z.string().max(200).optional().transform(val => val ?? ''),
  ltv: z.string().max(200).optional().transform(val => val ?? ''),
  ...
  hiringPlan: z.string().max(1000).optional().transform(val => val ?? ''),
  keyPartners: z.string().max(500).optional().transform(val => val ?? ''),

  // Step 8: Financials
  currentRevenue: z.string().max(200).optional().transform(val => val ?? ''),
  revenueTarget: z.string().min(5, 'Target ricavi richiesto').max(500),
  fixedCosts: z.string().min(5, 'Costi fissi richiesti').max(500),
  margins: z.string().max(200).optional().transform(val => val ?? ''),
  breakeven: z.string().max(200).optional().transform(val => val ?? ''),
  ...
  regulatoryIssues: z.string().max(500).optional().transform(val => val ?? ''),
  additionalNotes: z.string().max(2000).optional().transform(val => val ?? ''),
});
